#ifndef REDWALK_ADJACENCY_ARRAY_H
#define REDWALK_ADJACENCY_ARRAY_H
...
{
public:
  typedef redwalk::vid_t vid_t;
  typedef redwalk::idx_t idx_t;
...
public:
  NeighborhoodList() : beg_(RW_NULLPTR), end_(RW_NULLPTR) { }
...
public:
    ...
    edge_end, bool col_mjr)
  {
    ...
    //adj_ = redwalk_alloc<vid_t, idx_t>(2*ne_);
    adj_ = new vid_t[2*ne_];
    //vtx_ = redwalk_alloc<vid_t*, idx_t>(nv_+1);
    vtx_ = new vid_t*[nv_+1];

    //Set up degrees
    //idx_t* deg = redwalk_alloc<idx_t, idx_t>(nv_);
  ...
  #ifdef _RW_CXX11 //C++11 Move Semantics
  AdjacencyArray(AdjacencyArray&& mov)
  {
    nv_ = mov.nv_; mov.nv_ = 0;
    ne_ = mov.ne_; mov.ne_ = 0;
    adj_ = mov.adj_; mov.adj_ = RW_NULLPTR;
    vtx_ = mov.vtx_; mov.vtx_ = RW_NULLPTR;
  }

  AdjacencyArray& operator=(AdjacencyArray&& mov)
  {
    if(adj_ != RW_NULLPTR) { delete [] adj_; }
    if(vtx_ != RW_NULLPTR) { delete [] vtx_; }
    nv_ = mov.nv_; mov.nv_ = 0;
    ne_ = mov.ne_; mov.ne_ = 0;
    adj_ = mov.adj_; mov.adj_ = RW_NULLPTR;
    vtx_ = mov.vtx_; mov.vtx_ = RW_NULLPTR;
  ...
  #endif//_RW_CXX11 (C++11 Move Semantics)
...
#endif //REDWALK_ADJACENCY_ARRAY_H
